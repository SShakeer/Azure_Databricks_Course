...
# DBTITLE 1,Window Functions (OLAP Functions)
...
# MAGIC %sql
# MAGIC select deptno,sal,ename,rank() over (partition by deptno order by sal desc) as rank from emp;
...
#Write PySpark program to display 2 hihest salary in each dept?
...
spec0 = Window.partitionBy("deptno").orderBy(desc("sal"))

df_rank=emp_df.withColumn("rank1",rank().over(spec0))

df_filter=df_rank.filter(col("rank1")==1).display()
...
#spec0 = Window.partitionBy("deptno").orderBy(desc("sal"))
df_rank=emp_df.withColumn("rank1",dense_rank().over(Window.partitionBy("deptno").orderBy(desc("sal")))).filter(col("rank1")==2).show()

#df_filter=df_rank.filter(col("rank1")==2).show()
...
# MAGIC %sql
# MAGIC select ename,sal,deptno,hiredate,row_number() over (order by sal desc) as sno from emp
...
df4=df3.where(col("rank1")==1).display()
...
df2=df.groupBy(df.dept).max("salary").alias("maxsalary")
df2.show()

# COMMAND ----------

# DBTITLE 1,Using Window Functions 
...
dbutils.fs.put("/tmp/dataframe_sample02412.csv", """id|end_date|start_date|location
...
df = spark.read.format("csv").options(header='true', delimiter = '|').load("/tmp/dataframe_sample2024.csv")

# Convert to a Date type
df2 = df.withColumn('date', F.to_date(df.end_date))

# Parse out the date only
df3 = df2.withColumn('date_only', F.regexp_replace(df2.end_date,' (\d+)[:](\d+)[:](\d+).*$', ''))
...
df = df3.withColumn('state', F.split(df3.location, '-')[0]).withColumn('city', F.split(df3.location, '-')[1]).display()

# COMMAND ----------

dbutils.fs.ls("/tmp/dataframe_sample2024.csv")
...
dbutils.fs.put("/tmp/dataframe_sample2024.csv", """id|end_date|start_date|location
1|2015-10-14 00:00:00|2015-09-14 00:00:00|CA-SF
2|2015-10-15 01:00:20|2015-08-14 00:00:00|CA-SD
3|2015-10-16 02:30:00|2015-01-14 00:00:00|NY-NY
4|2015-10-17 03:00:20|2015-02-14 00:00:00|NY-NY
...
# Split a string and index a field
df = df.withColumn('city', F.split(df.location, '-')[0]).withColumn('city2', F.split(df.location, '-')[1]).show()
...
df = df3.withColumn('end_month', F.month('end_date'))
df = df3.withColumn('end_year', F.year('end_date')).display()
...
#    -- count(1) OVER (PARTITION BY deptno) AS employee_count,
#     --rank() OVER (ORDER BY sal DESC) AS rnk,
#     lead(sal) OVER (PARTITION BY deptno ORDER BY sal DESC) AS lead_emp_sal,
#     lag(sal) OVER (PARTITION BY deptno ORDER BY sal DESC) AS lag_emp_sal
...
df_sum=emp_df.groupBy("deptno").sum("sal").show()
...
# MAGIC %sql
# MAGIC SELECT t.empno,t.ename,t.sal,
...
spec = Window.partitionBy("deptno").orderBy(desc("sal"))
df3=emp_df.withColumn("prior_revenue",lead('sal',1,0).over(spec)).withColumn("post_revenue",lag('sal',1,0).over(spec)).display()
...
df3=df2.withColumn("rnk",rank().over(spec)).\
  withColumn("drnk",dense_rank().over(spec)).display()
...
# MAGIC %sql
# MAGIC SELECT 
# MAGIC         nvl(deptno,"GrandTotal") as deptno, SUM(sal) AS TotalSales
# MAGIC     FROM 
# MAGIC         emp
# MAGIC     GROUP BY 
# MAGIC         ROLLUP(deptno)
...
# MAGIC %sql
# MAGIC select deptno,sum(sal) as Total from emp group by rollup(deptno)

# COMMAND ----------

emp_df.cube("deptno","job").agg(sum("sal")).show()
#emp_df.rollup("deptno").agg(sum("sal").alias("Total")).show()
...
# MAGIC select e.* from
# MAGIC (
# MAGIC select empno,sal,deptno,dense_rank() over (partition by deptno order by sal desc) as rnk from emp
# MAGIC ) e
# MAGIC where e.rnk=2
# MAGIC
