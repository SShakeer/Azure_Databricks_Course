...
emp_df. \
    select('ename',lower(col("ename")).alias("uename"), initcap(col("job")).alias("newjob"),upper(col("job")).alias("upper_conv")). \
    show()

# COMMAND ----------

df=spark.sql("select lower(ename) as lname,initcap(job) as job,upper(job) as ujob from emp").show()

# COMMAND ----------

df100=emp_df.select("ename")
df101=df100.withColumn("lname",lower(col("ename"))).show()
...
#When ever you want to cmpare multiple conditions , we will use when otherwise

#it is similar to CASE in SQL
from pyspark.sql.functions import * 
...
# MAGIC %sql
# MAGIC select empno,ename,
# MAGIC case job
# MAGIC when "CLERK" then "office work"
# MAGIC when "MANAGER" then "He is manager"
# MAGIC when "PRESIDENT" then "He is pres"
# MAGIC when "SALESMAN" then "salesman"
# MAGIC else job 
# MAGIC end as duty
# MAGIC from emp;
# MAGIC
...
from pyspark.sql.functions import *
df2 = emp_df.withColumn("abc",when(col("job") == 'CLERK','Male').when(col("job") == 'PRESIDENT','Female').otherwise('NO gender')).show()
