...
emp_df.write.mode("overwrite").format("csv").save("/FileStore/tables/emp2024072.csv")
#overwrite,append
...
dbutils.fs.ls("/FileStore/tables/emp2024072.csv")

# COMMAND ----------

emp_df.coalesce(1).write.mode("overwrite").format("csv").save("/FileStore/tables/emp2024072.csv")
...
dbutils.fs.ls("/FileStore/tables/emp20240729.csv")
...
Deptno,dname,loc
Deptno,dname,loc,city

# COMMAND ----------

# MAGIC %sql
# MAGIC create database azure;

# COMMAND ----------

dept_df.write.format("delta").partitionBy("deptno").mode("overwrite").option("mergeSchema","true").saveAsTable("azure.dept2025")

# COMMAND ----------

#"hivemetastore.azure.dept2024"
...
dbutils.fs.ls("/user/hive/warehouse/azure.db/dept2024")
...
df=spark.read.csv(f'{path}', header=True).show()
...
output_dir="/FileStore/tables/output2024"
...
df_add.write.mode("overwrite").saveAsTable("azure.emp")
